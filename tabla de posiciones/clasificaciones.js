document.addEventListener('DOMContentLoaded', function() {
    ...
    const clasificaciones = [
      { nombre: 'Usuario', imagen: '', puntuacion: puntajeUsuario },
    ...
    clasificaciones.forEach(function(clasificacion, index) {
        ...
        if (filaExistente) {
          ...
          filaExistente.innerHTML = `<td>${index + 1}</td><td>${clasificacion.nombre}</td><td>${clasificacion.puntuacion}</td><td><img src="${clasificacion.imagen}" alt="${clasificacion.nombre}" width="50" height="50"></td>`;
